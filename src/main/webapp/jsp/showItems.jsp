...
<div class="container syl">
    ...
    <div class="show-img">
        <a><img src="${item.itempic }"></a>
    </div>
    <div class="ifo-img">
        <h1>${item.itemname }</h1>
        <p>转  卖  价：<b>¥<em>${item.price }</em></b></p>
        <p>原　　价：<b>¥</b> ${item.price+50}</p>
        <hr>
        <p>成　　色：<span>${item.levels}</span>成新</p>
        <p>所  在  地：<span>${shopn.shopaddr}111</span></p>
        <p>店        铺：<a href="selectallitem?shopid=3333333333"><span>${shopn.shopname}111</span></a></p>
        <p>交易方式：<span>在线交易</span></p>
        <div>
        <button id="add"><span class="glyphicon glyphicon-shopping-cart"></span> 加入购物车</button>
        <input type="hidden" value="${item.itemid}">
        </div>
        <div><img width="99%" src="img/tup2.bmp"/></div>
    </div>
    </div>
    <div class="nav1">
        ...
        <div class="top val-one">
            <h3>宝贝介绍</h3>
            <p>${item.iteminfo}全新正品带吊阿迪达斯球衣，亚版s码，适合身高175cm左右，体型不是太壮的球友，本人175，130斤穿s合适，可参考。自己收藏的sw球衣出一波，已经出了一些了，留了几件喜欢的自穿，都是单品，所以见到即缘分，本人不碰假，如假白送，鉴别细节图基本都给了，有想法还要细节的可私。所标均为实价，欢迎比价，所以请不要上来就最低价，有诚意就带上自己的价，只小刀谢谢，动辄百八十刀的烦您再看看，可能缘分未到，出不了就继续收着，也不是急需钱。</p>
        </div>
            ...
            <div class="huif">
                <div class="input-group input-group-lg">
                    ...
                    <span class="input-group-btn">
                        <button id="btn1"
								onclick="return message(${sessionScope.users.usersid},${item.itemid})"
